...
napi_value call_roc(napi_env env, napi_callback_info info) {
  ...
  // roc_panic needs a napi_env in order to throw a Node exception, so we
  // provide this one globally in case roc_panic gets called during the
  // execution of our Roc function.
  //
  ...
  // native function" is the one that's "passed to any subsequent nested
  // Node-API calls," so we must override this every time we call this function
  // (as opposed to, say, setting it once during init).
  napi_global_env = env;
...
napi_value init(napi_env env, napi_value exports) {
  ...
  status = napi_set_named_property(env, exports, "main", fn);
